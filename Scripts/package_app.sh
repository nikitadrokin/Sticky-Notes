...
BUNDLE_ID=${BUNDLE_ID:-com.dna.stickynotes}
...
# SwiftPM path layout varies by toolchain:
# - Classic: .build/<arch>-apple-macosx/<config>/<name>
# - Swift 6.4+ (Native Build System): .build/<config>/<name>
build_product_path() {
  local name="$1"
  local arch="$2"
  local candidates=(
  ...
  for candidate in "${candidates[@]}"; do
    if [[ -f "$candidate" ]]; then
      # Shared config path may hold a different arch after a prior build.
      if command -v lipo >/dev/null 2>&1; then
        local actual_arches
        actual_arches=$(lipo -archs "$candidate" 2>/dev/null || true)
        if [[ -n "$actual_arches" && "$actual_arches" != *"$arch"* ]]; then
          continue
        fi
      fi
      echo "$candidate"
...
FRAMEWORK_DIRS=("$PREFERRED_BUILD_DIR" ".build/$CONF" ".build/${ARCH_LIST[0]}-apple-macosx/$CONF")
for dir in "${FRAMEWORK_DIRS[@]}"; do
  if [[ -d "$dir" ]] && compgen -G "${dir}/*.framework" >/dev/null; then
